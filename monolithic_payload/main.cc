...
void TestQueueStacks(FileHandler* sq_exerciser) {
  std::cout << "\n--->>--->> main::TestQueueStacks <<---<<--- " << std::endl;
  sq_exerciser->TestQueueStacks();
}

void TestCircularQueue(FileHandler* sq_exerciser) {
  std::cout << "\n--->>--->> main::TestCircularQueue <<---<<--- " << std::endl;
  sq_exerciser->TestCircularQueue();
}

void TestMaxElement(FileHandler* sq_exerciser) {
...
void BinaryTreeLevelOrder(FileHandler* sq_exerciser) {
  /*********************************
          Binary Tree to Test
                  3
                /   \
...
void IsWellFormedBrackets(FileHandler* sq_exerciser) {
...
void EvaluateRPN(FileHandler* sq_exerciser) {
...
void RunTests(FileHandler* sq_exerciser) {
...
int main(int argc, char const *argv[]) {
    std::shared_ptr<FileHandler> sq_exerciser(new FileHandler());
