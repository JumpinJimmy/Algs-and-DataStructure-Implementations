...
// kSIZE_MASK
// kSIZE_OFFSET
bool FileHandler::MarshalValue(const std::string& payload_string, int type) {
    std::stringstream encode_sstream(std::ios_base::out | std::ios_base::ate);
    uint8_t header_byte = 0;
    uint8_t header_bit_field = 0;
    std::size_t payload_size = payload_string.size();
    std::cout << "payload_size: " << payload_size << " \n type: " << type << std::endl;
    header_bit_field = (type & kTYPE_MASK) << kTYPE_OFFSET;
    header_bit_field |= (payload_size & kSIZE_MASK);
    std::cout << "--->>--->> file_handler:: Marshal Value: header bit field: " << std::dec << int(header_bit_field)  << std::endl;
    return false;
}
// int FileHandler::EvaluateRPN(const std::string& RPN_expression) {
...
// bool FileHandler::IsWellFormedBrackets(const std::string& bracket_str) {
...
// std::vector<std::vector<int>> FileHandler::BinTreeLevelOrder(const std::unique_ptr<BinaryTreeNode<int>>& tree) { //NOLINT
//     if (!tree) {
...
// /// Helper Methods
// void FileHandler::TestMaxStack(const std::vector<int> &elements) {
//     MaxStack test_stack;
...
// void FileHandler::TestStackScope(const std::shared_ptr<MaxStack> &s) {
//     std::cout << "--->>--->> TestStackScope s->Top(): " << s->Top()
//               << ", s->Max(): " << s->Max()
//               << ", s->stack_size(): " << s->stack_size() << std::endl;
// }

// std::shared_ptr<MaxStack> FileHandler::CreateStack(int n) {
...
// std::shared_ptr<MaxStack> FileHandler::CreateStack(const std::vector<int> &elements) {
...
// void FileHandler::TestCircularQueue() {
...
// void FileHandler::TestQueueStacks() {
//     QueueStacks qs;
